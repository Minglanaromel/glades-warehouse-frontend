...
const PurchaseReport = () => {
  ...
  const topSuppliersChart = {
    labels: data.topSuppliers.map(s => s.name),
    datasets: [
      {
        label: 'Purchase Amount',
        data: data.topSuppliers.map(s => s.amount),
  ...
  const exportToExcel = () => {
    const exportData = data.daily.map(day => ({
      'Date': day.date,
      'Orders': day.count,
      'Purchase Amount': day.amount,
      'Items Purchased': day.items,
    }));

    const ws = XLSX.utils.json_to_sheet(exportData);
    const wb = XLSX.utils.book_new();
    XLSX.utils.book_append_sheet(wb, ws, 'Purchase Report');
    XLSX.writeFile(wb, `purchase_report_${dateRange.startDate}_to_${dateRange.endDate}.xlsx`);
  };
  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <h1 className="text-2xl font-semibold text-gray-900">Purchase Report</h1>
        <button
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
        <div className="bg-white rounded-lg shadow p-6">
          ...
          <p className="text-3xl font-semibold text-gray-900">
            ${data.summary.totalPurchases?.toFixed(2) || '0.00'}
          </p>
        </div>
        <div className="bg-white rounded-lg shadow p-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div className="bg-white rounded-lg shadow p-6">
          ...
          <Line data={dailyPurchasesChart} options={chartOptions} />
        </div>
        ...
        <div className="bg-white rounded-lg shadow p-6">
          ...
          <Bar data={topSuppliersChart} options={chartOptions} />
        </div>
      </div>
        ...
        <table className="min-w-full divide-y divide-gray-200">
          <thead className="bg-gray-50">
            <tr>
              ...
              <th className="table-header">Items Purchased</th>
              <th className="table-header">Purchase Amount</th>
