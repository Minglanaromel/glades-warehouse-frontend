...
const MachineStatus = () => {
  ...
  const getStatusColor = (status) => {
    ...
    if (statusLower.includes('running')) return 'bg-green-100 text-green-800 border-green-200';
    if (statusLower.includes('idle')) return 'bg-yellow-100 text-yellow-800 border-yellow-200';
    if (statusLower.includes('breakdown')) return 'bg-red-100 text-red-800 border-red-200';
    if (statusLower.includes('maintenance')) return 'bg-purple-100 text-purple-800 border-purple-200';
  ...
  const getStatusIcon = (status) => {
    ...
    const statusLower = status.toLowerCase();
    if (statusLower.includes('running')) return <CheckCircleIcon className="h-5 w-5 text-green-600" />;
    if (statusLower.includes('idle')) return <ClockIcon className="h-5 w-5 text-yellow-600" />;
    if (statusLower.includes('breakdown')) return <XCircleIcon className="h-5 w-5 text-red-600" />;
    if (statusLower.includes('maintenance')) return <WrenchScrewdriverIcon className="h-5 w-5 text-purple-600" />;
    return null;
  };

  const stats = {
    total: machines.filter(m => m).length,
    running: machines.filter(m => m && m.status && m.status.toLowerCase().includes('running')).length,
    idle: machines.filter(m => m && m.status && m.status.toLowerCase().includes('idle')).length,
    breakdown: machines.filter(m => m && m.status && m.status.toLowerCase().includes('breakdown')).length,
      ...
      <div className="bg-gradient-to-r from-blue-900 via-blue-800 to-blue-900 text-white p-6 rounded-lg shadow-lg">
        <div className="flex items-center space-x-4">
          ...
          <div>
            ...
            <p className="text-blue-200 text-sm mt-1">Machine Status Monitor • Real-time Dashboard</p>
          </div>
        </div>
        ...
        <div className="flex flex-wrap justify-between items-center mt-4 pt-4 border-t border-blue-700">
          <div className="flex items-center space-x-4">
            <div className="flex items-center space-x-2 bg-blue-800/50 px-4 py-2 rounded-lg">
              ...
              <span className="text-sm text-blue-200">Last Update: {lastUpdate?.toLocaleTimeString() || 'N/A'}</span>
            </div>
          </div>
            ...
            <tbody className="bg-white divide-y divide-gray-200">
              {filteredMachines.map((machine, index) => (
                <tr key={index} className="hover:bg-gray-50">
                  ...
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{machine?.process || 'N/A'}</td>
                  <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">{machine?.machine || 'N/A'}</td>
                  <td className="px-6 py-4 whitespace-nowrap">
                    <div className="flex items-center">
